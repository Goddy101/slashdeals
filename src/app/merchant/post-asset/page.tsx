...
export default function PostDigitalAsset() {
  ...
  const [formData, setFormData] = useState({
    ...
    follower_count: '',
    engagement_rate: '',
    includes_og_email: false,

    // Proof of Ownership
    ownership_proof_method: 'dns_txt'
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Pack the dynamic fields into our new JSONB schema
      const assetMetrics = isSocial ? {
        platform: formData.platform,
        followers: Number(formData.follower_count) || 0,
        engagement_rate: Number(formData.engagement_rate) || 0,
        includes_og_email: formData.includes_og_email,
        proof_method: formData.ownership_proof_method
      } : {
        mrr: Number(formData.monthly_revenue) || 0,
        users: Number(formData.monthly_traffic) || 0,
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-white p-6 rounded-[24px] border border-gray-200 shadow-sm space-y-6">
            ...
            {parentCategory === 'startup' ? (
              ...
              <>
                ...
                <div className="flex items-center gap-3 mt-4 p-4 bg-gray-50 rounded-xl border border-gray-200">
                  <input type="checkbox" name="includes_og_email" checked={formData.includes_og_email} onChange={handleCheckboxChange} className="w-5 h-5 text-pink-600 rounded" />
                  <label className="text-sm font-bold text-gray-700">Original Creation Email is included in the sale (Highly recommended)</label>
                </div>
              </>
            )}
          </div>
          ...
          <div className="bg-white p-6 rounded-[24px] border border-gray-200 shadow-sm space-y-4">
            ...
            <p className="text-sm text-gray-500 mb-4">Buyers will not purchase unverified assets. How will you prove you own this?</p>
            
            <select name="ownership_proof_method" value={formData.ownership_proof_method} onChange={handleInputChange} className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:ring-2 focus:ring-black outline-none font-medium">
              {parentCategory === 'startup' ? (
                <>
                  <option value="dns_txt">I will add a TXT record to my Domain DNS</option>
                  <option value="google_analytics">I will grant View Access to Google Analytics / Stripe</option>
                </>
              ) : (
                <>
                  <option value="bio_code">I will paste a verification code in my Social Bio</option>
                  <option value="live_screen_share">I will do a live screen-share with a SlashDeals Admin</option>
                </>
              )}
            </select>
          </div>

          {/* 5. Monetization Upsell: The Due Diligence Badge */}
          ...
          <p className="text-center text-xs text-gray-400 font-medium">
            By listing this asset, you agree to the SlashDeals 5-Day Digital Escrow Handover Protocol.
          </p>
