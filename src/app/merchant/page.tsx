...
export default async function MerchantDashboard() {
  ...
  const organicCtr = organicViews > 0 ? ((organicClicks / organicViews) * 100).toFixed(2) : '0.00';
  ...
  const calculatedCtr = totalAdViews > 0 ? ((totalAdClicks / totalAdViews) * 100).toFixed(2) : '0.00';
  ...
  if (dealIds.length > 0) {
    ...
    if (analytics) {
      analytics.forEach(event => {
        ...
        if (dayIndex !== -1) {
          if (event.event_type === 'view') chartData[dayIndex].views += 1;
          else if (event.event_type === 'click') chartData[dayIndex].clicks += 1;
        }
      });
    }
  }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      {/* 🚀 NEW: Quick Actions / Bento Box */}
      <div className="mb-8">
        <h2 className="text-sm font-black text-gray-400 uppercase tracking-wider mb-4">Quick Actions</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          
          {/* Physical Goods Action */}
          <Link 
            href="/merchant/post" 
            className="group bg-white p-5 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md hover:border-gray-300 transition-all flex items-start gap-4"
          >
            <div className="w-12 h-12 bg-gray-50 border border-gray-100 rounded-xl flex items-center justify-center text-xl shrink-0 group-hover:scale-110 transition-transform">
              📦
            </div>
            <div>
              <h3 className="font-black text-gray-900 group-hover:text-black transition-colors">Sell Physical Good</h3>
              <p className="text-xs text-gray-500 mt-1 font-medium leading-relaxed">
                Electronics, gadgets, real estate, and wholesale items.
              </p>
            </div>
          </Link>

          {/* Digital Assets Action (Premium) */}
          <Link 
            href="/merchant/post-asset" 
            className="group bg-gradient-to-br from-zinc-900 to-black p-5 rounded-2xl shadow-sm hover:shadow-xl transition-all flex items-start gap-4 relative overflow-hidden"
          >
            ...
            <div className="w-12 h-12 bg-white/10 border border-white/10 rounded-xl flex items-center justify-center text-xl shrink-0 group-hover:scale-110 transition-transform z-10">
              💻
            </div>
            <div className="z-10">
              <div className="flex items-center gap-2">
                <h3 className="font-black text-white">Sell Digital Asset</h3>
                <span className="bg-blue-500 text-white text-[9px] font-black uppercase px-2 py-0.5 rounded shadow-sm">Premium</span>
              </div>
              <p className="text-xs text-zinc-400 mt-1 font-medium leading-relaxed">
                SaaS, domains, and monetized social media accounts.
              </p>
            </div>
          </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white border border-gray-200 p-6 sm:p-8 rounded-xl shadow-sm relative overflow-hidden flex flex-col justify-between">
          <div className="relative z-10 flex justify-between items-start">
            <div>
              <p className="text-gray-500 font-bold uppercase tracking-wider text-sm mb-1">Organic Traffic Clicks</p>
              <h1 className="text-4xl sm:text-5xl font-black text-gray-900">{organicClicks.toLocaleString()}</h1>
            </div>
            <div className="text-right">
              <p className="text-gray-400 font-bold uppercase tracking-wider text-xs mb-1">Average CTR</p>
              <div className="inline-block bg-gray-100 text-gray-700 font-black text-xl px-3 py-1 rounded-xl shadow-sm border border-gray-200">{organicCtr}%</div>
            </div>
          </div>
          <div className="relative z-10 mt-4 flex justify-between items-end">
            <p className="text-sm text-gray-500">From {organicViews.toLocaleString()} total organic views.</p>
          </div>
        </div>
        ...
        <div className="bg-blue-600 text-white p-6 sm:p-8 rounded-xl shadow-md relative overflow-hidden flex flex-col justify-between">
          <div className="relative z-10 flex justify-between items-start">
            <div>
              <p className="text-blue-200 font-bold uppercase tracking-wider text-sm mb-1">Ad Campaign Clicks</p>
              <h1 className="text-4xl sm:text-5xl font-black text-white">{totalAdClicks.toLocaleString()}</h1>
            </div>
            <div className="text-right">
              <p className="text-blue-200 font-bold uppercase tracking-wider text-xs mb-1">Average CTR</p>
              <div className="inline-block bg-white text-blue-700 font-black text-xl px-3 py-1 rounded-xl shadow-sm">{calculatedCtr}%</div>
            </div>
          </div>
          <div className="relative z-10 mt-4 flex justify-between items-end">
            <p className="text-sm text-blue-100">From {totalAdViews.toLocaleString()} total ad views.</p>
            <Link href="/merchant/advertise" className="text-sm font-bold text-white hover:text-blue-200 underline">Run new ad &rarr;</Link>
          </div>
      ...
      {myAds && myAds.length > 0 && (
        <div className="bg-white rounded-xl border border-blue-200 shadow-sm overflow-hidden mb-8">
          <div className="bg-blue-50 border-b border-blue-100 p-6 flex justify-between items-center">
            <div>
              <h3 className="text-lg font-black text-blue-900 flex items-center gap-2">🚀 Active Category Monopolies</h3>
              <p className="text-sm text-blue-700">Track your exclusive advertising ROI.</p>
            </div>
            <Link href="/merchant/advertise" className="text-sm font-bold text-blue-600 hover:underline">Buy Another Slot →</Link>
          </div>
          
          <div className="p-6">
            <div className="grid grid-cols-1 gap-6">
              {myAds.map((ad: any) => {
                const ctr = ad.total_views > 0 ? ((ad.total_clicks / ad.total_views) * 100).toFixed(2) : '0.00';
                  ...
                  <div key={ad.id} className="border border-gray-100 rounded-2xl p-5 flex flex-col md:flex-row items-center gap-6 bg-gray-50">
                    ...
                    <div className="flex-1 space-y-1 text-center md:text-left w-full">
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
                        <span className="bg-black text-white text-[10px] font-black uppercase px-2 py-1 rounded tracking-widest inline-block w-fit">{ad.category} Monopoly</span>
                        <span className={`text-xs font-bold ${daysLeft > 5 ? 'text-green-600' : 'text-red-500'}`}>{daysLeft > 0 ? `${daysLeft} days remaining` : 'Expired'}</span>
                      </div>
                    ...
                    <div className="flex items-center justify-between gap-8 w-full md:w-auto bg-white p-4 rounded-xl border border-gray-200">
                      <div className="text-center"><p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Views</p><p className="text-2xl font-black text-gray-900">{ad.total_views || 0}</p></div>
                      <div className="text-center"><p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Clicks</p><p className="text-2xl font-black text-blue-600">{ad.total_clicks || 0}</p></div>
                      <div className="text-center hidden sm:block"><p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">CTR</p><p className="text-2xl font-black text-emerald-500">{ctr}%</p></div>
                    </div>
                  </div>
                );
              })}
                  ...
                  return (
                    <tr key={deal.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 flex items-center justify-end gap-2">
                        ...
                        <PromoteDealAction dealId={deal.id} dealTitle={deal.title} currentBalance={walletBalance} />
                      </td>
                    </tr>
                  );
                })}
